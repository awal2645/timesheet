        ...
        <form method="POST" action="{{ route('task.update', $task->id) }}" class="max-w-md mx-auto">
            ...
            <div class="relative z-0 w-full mb-5 group">
                <select name="employer_id" id="employer_id"
                    ...
                    @foreach ($employers as $employer)
                        <option class="dark:bg-slate-800" value="{{ $employer->id }}" {{ $task->employer_id == $employer->id ? 'selected' : '' }}>
            ...
            <div class="relative z-0 w-full mb-5 group">
                <select name="employee_id" id="employee_id"
                    ...
                    @foreach ($employees as $employee)
                        <option class="dark:bg-slate-800" value="{{ $employee->id }}" {{ $task->employee_id == $employee->id ? 'selected' : '' }}>
            ...
            <div class="relative z-0 w-full mb-5 group">
                <select name="project_id" id="project_id"
                    ...
                    @foreach ($projects as $project)
                        <option class="dark:bg-slate-800" value="{{ $project->id }}" {{ $task->project_id == $project->id ? 'selected' : '' }}>
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" name="time" id="time" value="{{ $task->time }}"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="0:00" />
            ...
            <div class="relative z-0 w-full mb-5 group">
                <select name="priority" id="priority"
                    ...
                    <option class="dark:bg-slate-800" value="low" {{ $task->priority == 'low' ? 'selected' : '' }}>{{ __('Low Priority') }}</option>
                    <option class="dark:bg-slate-800" value="medium" {{ $task->priority == 'medium' ? 'selected' : '' }}>{{ __('Medium Priority') }}</option>
                    <option class="dark:bg-slate-800" value="high" {{ $task->priority == 'high' ? 'selected' : '' }}>{{ __('High Priority') }}</option>
                </select>
                @error('priority')
                <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            ...
            <div class="relative z-0 w-full mb-5 group">
                <select name="status" id="status"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                    <option class="dark:bg-slate-800" value="pending" {{ $task->status == 'pending' ? 'selected' : '' }}>{{ __('Pending') }}</option>
                    <option class="dark:bg-slate-800" value="inprogress" {{ $task->status == 'inprogress' ? 'selected' : '' }}>{{ __('In Progress') }}</option>
                    <option class="dark:bg-slate-800" value="completed" {{ $task->status == 'completed' ? 'selected' : '' }}>{{ __('Completed') }}</option>
                </select>
                @error('status')
                <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            ...
            {{-- Submit Button --}}
            <div class="relative z-0 w-full mb-5 group">
                <button type="submit" class="w-full text-white bg-blue-500 hover:bg-blue-600 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700">
                    {{ __('Update Task') }}
                </button>
            </div>
        </form>
