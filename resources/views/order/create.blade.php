...
<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-slate-800 shadow-2xl rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Complete all required fields to create your order. All fields marked with * are mandatory.') }}
                        </p>
                    </div>
            ...
            <!-- Form Content -->
            <div class="p-8">
                <form method="POST" action="{{ route('order.store') }}" class="space-y-8" id="orderForm">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                <select name="employer_id" id="employer_id" 
                                    ...
                                    <option value="" disabled selected>{{ __('Choose an employer...') }}</option>
                        ...
                        <!-- Plan Selection -->
                        <div class="space-y-2">
                            <label for="plan_id" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                <span class="flex items-center">
                        ...
                        <!-- Payment Method -->
                        <div class="space-y-2">
                            <label for="payment_method" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                <span class="flex items-center">
                            ...
                            <div class="relative group">
                                <select name="payment_method" id="payment_method" 
                                        class="w-full px-4 py-4 border-2 border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 appearance-none hover:border-gray-400 dark:hover:border-gray-500 group-hover:shadow-md">
                                    <option value="" disabled selected>{{ __('Choose payment method...') }}</option>
                                    <option value="Paypal">💳 {{ __('PayPal') }}</option>
                                    <option value="Stripe">💰 {{ __('Stripe') }}</option>
                                    <option value="Offline">🏦 {{ __('Offline Payment') }}</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none">
                                    <svg class="w-5 h-5 text-gray-400 transition-colors duration-200 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                <select name="payment_status" id="payment_status" 
                                        class="w-full px-4 py-4 border-2 border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 appearance-none hover:border-gray-400 dark:hover:border-gray-500 group-hover:shadow-md">
                                    <option value="" disabled selected>{{ __('Select payment status...') }}</option>
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-sm text-blue-700 dark:text-blue-400">
                                    {{ __('Make sure to select the correct employer and plan. The payment method will determine how the transaction is processed, and you can always update the payment status later.') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between pt-8 border-t border-gray-200 dark:border-gray-600 space-y-4 sm:space-y-0">
                        ...
                        <button type="submit" id="submitBtn"
                            ...
                            <span class="submit-text">{{ __('Create Order') }}</span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function() {
                ...
                submitText.textContent = '{{ __("Creating Order...") }}';
                loadingSpinner.classList.remove('hidden');
                submitBtn.classList.add('opacity-75');
            });

            // Real-time form validation feedback
            const requiredFields = ['employer_id', 'plan_id', 'payment_method', 'payment_status'];
