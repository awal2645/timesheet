    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="mb-6 lg:mb-0">
                        ...
                        <p class="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
                            {{ __('Create, manage, and track invoices for your clients and projects with comprehensive billing management') }}
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 text-center">
                            <div class="text-2xl font-bold text-green-600 dark:text-green-400 mb-1">
                                {{ $invoices->unique('client_id')->count() }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                {{ __('Unique Clients') }}
                            </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 text-center">
                            <div class="text-2xl font-bold text-purple-600 dark:text-purple-400 mb-1">
                                {{ $invoices->unique('project_id')->count() }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                {{ __('Active Projects') }}
                            </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 text-center">
                            <div class="text-2xl font-bold text-orange-600 dark:text-orange-400 mb-1">
                                {{ $invoices->filter(function($invoice) {
                                    return $invoice->created_at->isCurrentMonth();
                                })->count() }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                {{ __('This Month') }}
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-8">
                ...
                <form action="{{ route('invoice.index') }}" method="GET" class="p-6">
                    <!-- Main Search Row -->
                        ...
                        <div class="lg:col-span-8">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" 
                                       ...
                                       placeholder="{{ __('Search by invoice number, client name, or project...') }}" />
                            </div>
                        </div>
                    ...
                    <div id="advancedFilters" class="hidden border-t border-gray-200 dark:border-gray-700 pt-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select name="client_filter" id="client_filter" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="">{{ __('All Clients') }}</option>
                                    @foreach($invoices->unique('client_id') as $invoice)
                                        @if($invoice->client)
                                            <option value="{{ $invoice->client->id }}" {{ request('client_filter') == $invoice->client->id ? 'selected' : '' }}>
                            ...
                            <!-- Project Filter -->
                            <div>
                                <label for="project_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('Filter by Project') }}
                                </label>
                                <select name="project_filter" id="project_filter" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="">{{ __('All Projects') }}</option>
                                    @foreach($invoices->unique('project_id') as $invoice)
                                        @if($invoice->project)
                                            <option value="{{ $invoice->project->id }}" {{ request('project_filter') == $invoice->project->id ? 'selected' : '' }}>
        ...
        <div>
                                ...
                                <select name="sort_by" id="sort_by" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="invoice_number" {{ request('sort_by') == 'invoice_number' ? 'selected' : '' }}>{{ __('Invoice Number') }}</option>
                                    <option value="created_at" {{ request('sort_by') == 'created_at' ? 'selected' : '' }}>{{ __('Created Date') }}</option>
                                    <option value="client_name" {{ request('sort_by') == 'client_name' ? 'selected' : '' }}>{{ __('Client Name') }}</option>
                                    <option value="project_name" {{ request('sort_by') == 'project_name' ? 'selected' : '' }}>{{ __('Project Name') }}</option>
                                </select>
                            </div>
                        </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-start">
                                            <div class="flex-shrink-0 w-12 h-12">
                                                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center border-2 border-gray-200 dark:border-gray-600">
                                            ...
                                            <div class="ml-4 flex-1">
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300">
                                                        <div class="w-2 h-2 bg-indigo-500 rounded-full mr-1.5"></div>
                                                        {{ __('Invoice') }}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-sm font-medium text-gray-900 dark:text-white">
                                                    {{ $invoice->client->client_name ?? __('N/A') }}
                                                </span>
                                            </div>
                                            <div class="flex items-center">
                                                <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <span class="text-sm text-gray-600 dark:text-gray-400">
                                                    {{ $invoice->project->project_name ?? __('N/A') }}
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="space-y-2">
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ __('Created:') }} {{ $invoice->created_at ? $invoice->created_at->format('M d, Y') : __('Unknown') }}
                                            </div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ __('Last updated:') }} {{ $invoice->updated_at ? $invoice->updated_at->diffForHumans() : __('Unknown') }}
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm">{{ __('No invoices have been created yet. Start by creating your first invoice for a client project.') }}</p>
                                            <a href="{{ route('invoice.create') }}"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            quickFilters.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    switch(filter) {
                        case 'this-month':
                            params.set('created_this_month', '1');
                            break;
                        case 'recent':
            ...
            function updateActiveFilters() {
                ...
                if (clientFilter) {
                    ...
                    filters.push({ label: `Client: ${clientName}`, field: 'client_filter', value: clientFilter });
                }
                if (projectFilter) {
                    const projectSelect = document.getElementById('project_filter');
                    const projectName = projectSelect.options[projectSelect.selectedIndex].text;
                    filters.push({ label: `Project: ${projectName}`, field: 'project_filter', value: projectFilter });
                }
                if (sortBy && sortBy !== 'invoice_number') {
