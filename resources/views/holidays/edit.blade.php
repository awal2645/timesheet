...
<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-slate-800 shadow-2xl rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700">
            <!-- Form Header -->
            <div class="bg-gradient-to-r from-red-50 to-pink-100 dark:from-slate-700 dark:to-slate-800 px-8 py-8 border-b border-gray-200 dark:border-gray-600">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                            <div class="w-8 h-8 bg-red-100 dark:bg-red-900/30 rounded-lg flex items-center justify-center mr-4">
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Update holiday information. Changes will affect future scheduling and employee calendars.') }}
                        </p>
                    </div>
                    ...
                    <div class="hidden md:flex flex-col items-end space-y-2">
                        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <div class="bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 px-3 py-1 rounded-full font-semibold">
                                #{{ $holiday->id }}
                            </div>
                        </div>
                        <div class="text-xs text-gray-400 dark:text-gray-500">
                            {{ __('Current Date') }}: {{ $holiday->date }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Content -->
            <div class="p-8">
                <form method="POST" action="{{ route('holidays.update', $holiday->id) }}" class="space-y-8" id="editHolidayForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-gray-200 dark:border-gray-600 pb-4">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('Update the holiday name and date') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <!-- Holiday Date -->
                            <div class="space-y-2">
                                <label for="date" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                    <span class="flex items-center">
                    ...
                    <!-- Change Tracking -->
                    <div id="changeTracker" class="hidden bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6">
                        <div class="flex items-start">
                            <div class="flex-shrink-0">
                            ...
                            <div class="ml-4">
                                ...
                                <div id="changesList" class="text-sm text-red-700 dark:text-red-400">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-red-700 dark:text-red-400 space-y-2">
                                    <p class="flex items-start">
                                        ...
                                        {{ __('Changes will update employee calendars and schedules') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                    ...
                                    <p class="flex items-start">
                                        ...
                                        {{ __('Check for conflicts with other holidays before saving') }}
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function trackChanges() {
            ...
            if (currentName !== originalName) {
                changes.push(`📝 {{ __('Name changed') }}: "${originalName}" → "${currentName}"`);
            }
            
            if (currentDate !== originalDate) {
                const oldDate = new Date(originalDate).toLocaleDateString('en-US', { 
                    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' 
                });
                const newDate = new Date(currentDate).toLocaleDateString('en-US', { 
                    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' 
                });
                changes.push(`📅 {{ __('Date changed') }}: ${oldDate} → ${newDate}`);
            }
        ...
        dateInput.addEventListener('change', trackChanges);
        ...
        window.setNextWeek = function() {
            const nextWeek = new Date();
            nextWeek.setDate(nextWeek.getDate() + 7);
            dateInput.value = nextWeek.toISOString().split('T')[0];
            dateInput.dispatchEvent(new Event('change'));
            showNotification('{{ __('Date set to next week') }}', 'success');
        };

        window.setNextMonth = function() {
            const nextMonth = new Date();
            nextMonth.setMonth(nextMonth.getMonth() + 1);
            dateInput.value = nextMonth.toISOString().split('T')[0];
            dateInput.dispatchEvent(new Event('change'));
            showNotification('{{ __('Date set to next month') }}', 'success');
        };
        ...
        document.addEventListener('keydown', function(e) {
            ...
            // Quick shortcuts for date presets
