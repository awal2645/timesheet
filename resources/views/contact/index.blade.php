...
<x-app-layout>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="mb-6 lg:mb-0">
                        ...
                        <p class="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
                            {{ __('Manage and respond to customer inquiries, contact requests, and communication messages from your website') }}
                        </p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-8">
                ...
                <form action="{{ route('contact.index') }}" method="GET" class="p-6">
                    <!-- Main Search Row -->
                        ...
                        {{ __('Search') }}
                            ...
                            <div class="relative">
                                ...
                                <input type="text" 
                                       ...
                                       placeholder="{{ __('Search by name, email, phone, or company...') }}" />
                            </div>
                        </div>
                    ...
                    <div id="advancedFilters" class="hidden border-t border-gray-200 dark:border-gray-700 pt-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- Company Filter -->
                            <div>
                                <label for="company_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('Filter by Company') }}
                                </label>
                                <select name="company_filter" id="company_filter" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="">{{ __('All Companies') }}</option>
                                    @foreach($contacts->unique('company')->pluck('company')->filter() as $company)
                            ...
                            <!-- Date Range -->
                            <div>
                                <label for="date_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('Filter by Date') }}
                                </label>
                                <select name="date_filter" id="date_filter" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                            ...
                            <div>
                                ...
                                <select name="sort_by" id="sort_by" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="created_at" {{ request('sort_by') == 'created_at' ? 'selected' : '' }}>{{ __('Latest First') }}</option>
                                    <option value="name" {{ request('sort_by') == 'name' ? 'selected' : '' }}>{{ __('Name A-Z') }}</option>
                                    <option value="company" {{ request('sort_by') == 'company' ? 'selected' : '' }}>{{ __('Company') }}</option>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($contacts as $contact)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                    <td class="px-6 py-4">
                                        <div class="flex items-start">
                                            <div class="flex-shrink-0 w-12 h-12">
                                                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center border-2 border-gray-200 dark:border-gray-600">
                                                    <span class="text-white font-semibold text-lg">{{ substr($contact->name, 0, 1) }}</span>
                                                </div>
                                            </div>
                                            <div class="ml-4 flex-1">
                                                ...
                                                <div class="space-y-1">
                                                    <div class="flex items-center">
                                                        ...
                                                        <span class="text-xs text-gray-600 dark:text-gray-400">{{ $contact->email }}</span>
                                                    </div>
                                                    @if($contact->phone)
                                                        <div class="flex items-center">
                                                            <svg class="w-3 h-3 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            ...
                                                            <span class="text-xs text-gray-600 dark:text-gray-400">{{ $contact->phone }}</span>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-2">
                                            <div class="text-sm text-gray-900 dark:text-white cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                                                        onclick="showFullMessage('{{ addslashes($contact->message) }}')">
                                                {{ Str::limit($contact->message, 60) }}
                                                @if(strlen($contact->message) > 60)
                                                    <span class="text-blue-600 dark:text-blue-400 ml-1">{{ __('Read more...') }}</span>
                                                @endif
                                            </div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ __('Received:') }} {{ $contact->created_at->format('M d, Y') }}
                                            </div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('No contact messages') }}</h3>
                                            <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm">{{ __('No contact inquiries have been received yet. When customers reach out through your website, their messages will appear here.') }}</p>
                                        </div>
                                                </td>
                                            </tr>
                            @endforelse
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            quickFilters.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    switch(filter) {
                        case 'today':
                            params.set('date_filter', 'today');
                            break;
                        case 'week':
                            params.set('date_filter', 'week');
                            break;
                        case 'recent':
            ...
            function updateActiveFilters() {
                ...
                const companyFilter = document.getElementById('company_filter')?.value || '';
                const dateFilter = document.getElementById('date_filter')?.value || '';
                const sortBy = document.getElementById('sort_by')?.value || '';

                if (search) filters.push({ label: `Search: ${search}`, field: 'search', value: search });
                if (companyFilter) {
                    filters.push({ label: `Company: ${companyFilter}`, field: 'company_filter', value: companyFilter });
                }
                if (dateFilter) {
                    const dateSelect = document.getElementById('date_filter');
                    const dateName = dateSelect.options[dateSelect.selectedIndex].text;
                    filters.push({ label: `Date: ${dateName}`, field: 'date_filter', value: dateFilter });
                }
        ...
        function showConfirmation(id) {
            Swal.fire({
                title: '{{ __("Want to delete this Contact!") }}',
