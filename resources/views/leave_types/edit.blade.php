...
<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-slate-800 shadow-2xl rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Update leave type information. Changes will affect all future leave requests using this type.') }}
                        </p>
                    </div>
                    ...
                    <div class="hidden md:flex flex-col items-end space-y-2">
                        ...
                        <div class="text-xs text-gray-400 dark:text-gray-500">
                            {{ __('Created') }}: {{ $leaveType->created_at ? $leaveType->created_at->format('M d, Y') : __('N/A') }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Content -->
            <div class="p-8">
                <form method="POST" action="{{ route('leave_types.update', $leaveType->id) }}" class="space-y-8" id="editLeaveTypeForm">
                    @csrf
                    @method('PUT')

                    <!-- Current Leave Type Info -->
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                <h3 class="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">
                                    {{ __('Current Leave Type Details') }}
                                </h3>
                                <div class="text-sm text-blue-700 dark:text-blue-400">
                                    <p><strong>{{ __('Type Name') }}:</strong> {{ $leaveType->type }}</p>
                                    <p><strong>{{ __('Description') }}:</strong> {{ $leaveType->description ?: __('No description provided') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-gray-200 dark:border-gray-600 pb-4">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('Update the leave type name and description') }}</p>
                        </div>
                            ...
                            <div class="space-y-2">
                                <label for="description" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                    <span class="flex items-center">
                                        ...
                                        {{ __('Description') }}
                                        <span class="text-gray-400 text-xs ml-2">({{ __('Optional') }})</span>
                                    </span>
                                </label>
                                <div class="relative group">
                                    <textarea name="description" id="description" rows="4"
                                              ...
                                              placeholder="Provide a detailed description of this leave type, including any specific conditions, eligibility criteria, or usage guidelines...">{{ old('description') ?? $leaveType->description }}</textarea>
                                </div>
                                @error('description')
                                <div class="flex items-center mt-2 p-3 text-red-700 bg-red-50 dark:bg-red-900/20 dark:text-red-400 rounded-lg border border-red-200 dark:border-red-800">
                                    <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-xl p-6 border border-gray-200 dark:border-gray-600">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <button type="button" onclick="useTemplate('Annual Leave', 'Yearly vacation time for rest and recreation. Typically accrued based on length of service.')" 
                                    class="px-4 py-3 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors text-sm font-medium text-left">
                                <div class="font-semibold">{{ __('Annual Leave') }}</div>
                                <div class="text-xs text-blue-600">{{ __('Vacation time') }}</div>
                            </button>
                            <button type="button" onclick="useTemplate('Sick Leave', 'Medical leave for illness, injury, or medical appointments. May require medical certification for extended periods.')" 
                                ...
                                <div class="font-semibold">{{ __('Sick Leave') }}</div>
                                <div class="text-xs text-green-600">{{ __('Medical absence') }}</div>
                            </button>
                            <button type="button" onclick="useTemplate('Maternity Leave', 'Leave for expectant mothers before and after childbirth. Duration and benefits may vary by policy and location.')" 
                                    class="px-4 py-3 bg-pink-100 text-pink-700 rounded-lg hover:bg-pink-200 transition-colors text-sm font-medium text-left">
                                <div class="font-semibold">{{ __('Maternity Leave') }}</div>
                                <div class="text-xs text-pink-600">{{ __('Childbirth leave') }}</div>
                            </button>
                            <button type="button" onclick="useTemplate('Emergency Leave', 'Unplanned leave for urgent personal or family emergencies. Requires manager approval and documentation.')" 
                                ...
                                <div class="font-semibold">{{ __('Emergency Leave') }}</div>
                                <div class="text-xs text-orange-600">{{ __('Urgent situations') }}</div>
                            </button>
                        </div>
                    </div>
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-orange-700 dark:text-orange-400 space-y-2">
                                    <p class="flex items-start">
                                        ...
                                        {{ __('Changes will affect all future leave requests using this type') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ __('Existing approved leave requests will remain unchanged') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ __('Consider notifying employees of any policy changes') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between pt-8 border-t border-gray-200 dark:border-gray-600 space-y-4 sm:space-y-0">
                        ...
                        <button type="submit" id="submitBtn"
                            ...
                            <span class="submit-text">{{ __('Update Leave Type') }}</span>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const originalType = '{{ $leaveType->type }}';
        const originalDescription = '{{ $leaveType->description }}';
        ...
        function trackChanges() {
            ...
            if (currentType !== originalType) {
                changes.push(`📝 {{ __('Type name changed') }}: "${originalType}" → "${currentType}"`);
            }
            
            if (currentDescription !== originalDescription) {
                const oldDesc = originalDescription || '{{ __('No description') }}';
                const newDesc = currentDescription || '{{ __('No description') }}';
                changes.push(`📄 {{ __('Description changed') }}: "${oldDesc}" → "${newDesc}"`);
            }
        ...
        typeInput.addEventListener('input', trackChanges);
        descriptionInput.addEventListener('input', trackChanges);

        // Template function
        window.useTemplate = function(typeName, description) {
            typeInput.value = typeName;
            descriptionInput.value = description;
            
            // Add visual feedback
            typeInput.classList.add('ring-2', 'ring-orange-500/20');
            descriptionInput.classList.add('ring-2', 'ring-orange-500/20');
            
            setTimeout(() => {
                typeInput.classList.remove('ring-2', 'ring-orange-500/20');
                descriptionInput.classList.remove('ring-2', 'ring-orange-500/20');
            }, 1000);
            
            showNotification(`{{ __('Template applied') }}: ${typeName}`, 'success');
            
            // Trigger change tracking
            trackChanges();
            
            // Focus on first field for potential editing
            typeInput.focus();
            typeInput.select();
        };
        ...
        function validateForm() {
            ...
            if (!typeInput.value.trim()) {
                typeInput.classList.add('border-red-500', 'bg-red-50', 'dark:bg-red-900/20');
                isValid = false;
            } else {
                typeInput.classList.remove('border-red-500', 'bg-red-50', 'dark:bg-red-900/20');
            }
        ...
        document.addEventListener('keydown', function(e) {
            ...
            // Quick shortcuts for templates
            if (e.ctrlKey || e.metaKey) {
                switch(e.key) {
                    case '1':
                        e.preventDefault();
                        useTemplate('Annual Leave', 'Yearly vacation time for rest and recreation. Typically accrued based on length of service.');
                        break;
                    case '2':
                        e.preventDefault();
                        useTemplate('Sick Leave', 'Medical leave for illness, injury, or medical appointments. May require medical certification for extended periods.');
                        break;
                    case '3':
                        e.preventDefault();
                        useTemplate('Maternity Leave', 'Leave for expectant mothers before and after childbirth. Duration and benefits may vary by policy and location.');
                        break;
                    case '4':
                        e.preventDefault();
                        useTemplate('Emergency Leave', 'Unplanned leave for urgent personal or family emergencies. Requires manager approval and documentation.');
                        break;
                }
            }
        });
