...
<x-app-layout>
    <div
        ...
        <a href="{{ route('leave_types.index') }}"
            class="btn bg-primary-500 dark:bg-primary-900 text-white">{{ __('Go to Leave Type List') }}</a>
    </div>
    <div
        ...
        <form method="POST" action="{{ route('leave_types.store') }}">
            ...
            <button type="submit"
                class="text-white bg-primary-500 dark:bg-primary-900 hover:bg-primary-600 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">{{ __('Submit') }}</button>
        </form>
    </div>
</x-app-layout>
