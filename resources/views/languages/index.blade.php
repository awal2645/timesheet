    ...
    <div class="relative overflow-x-auto">
        <div class="container mx-8">
            <div class="my-8 px-5 py-3 rounded-2xl dark:bg-black/10 bg-white/10 backdrop-blur border border-black/10 dark:border-white/10 flex flex-col md:flex-row justify-between items-center md:space-y-0 ">
                ...
                <a href="{{ route('languages.create') }}" class="bg-purple-500 text-white px-4 py-2 rounded-lg"><i class="fa-solid fa-plus"></i> {{ __('Create Language') }}</a>
            </div>
                                        ...
                                        <table class="w-full table-auto">
                                            <thead>
                                                <tr class="bg-gray-200 rounded-2xl text-left dark:bg-gray-700">
                                                    <th class="min-w-[220px] px-4 py-4 font-medium text-black dark:text-white xl:pl-11">{{ __('Language Name') }}</th>
                                                    <th class="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">{{ __('Language Code') }}</th>
                                                    <th class="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">{{ __('Direction') }}</th>
                                            ...
                                            <tbody>
                                                @if ($languagesList->count() > 0)
                                                    @foreach ($languagesList as $language)
                                                        <tr class="hover:bg-gray-100 hover:dark:bg-gray-800">
                                                            <td class="border-b border-[#eee] dark:border-slate-700 px-4 py-2.5">{{ $language->name }}</td>
                                                            <td class="border-b border-[#eee] dark:border-slate-700 px-4 py-2.5">{{ $language->code }}</td>
                                                            <td class="border-b border-[#eee] dark:border-slate-700 px-4 py-2.5">{{ __($language->direction) }}</td>
                                                            <td class="border-b border-[#eee] dark:border-slate-700 px-4 py-2.5">
                                                                <div class="flex justify-end">
                                                                    <a href="{{ route('languages.json.edit', $language->code) }}" class="text-blue-500 hover:text-blue-700">{{ __('Edit Json') }}</a>

                                                                    <a href="{{ route('languages.edit', $language->id) }}" class="text-blue-500 hover:text-blue-700">{{ __('Edit') }}</a>
                                                                    <form action="{{ route('languages.destroy', $language->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this language?');" class="ml-2">
                                                                        @csrf
                                                                        @method('DELETE')
                                                ...
                                                @else
                                                    <tr>
                                                        <td colspan="4" class="text-center py-8 border border-gray-300 dark:border-gray-700">
