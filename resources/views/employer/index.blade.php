                                    ...
                                    <table class="w-full table-auto">
                                        <thead>
                                            <tr class="bg-gray-200 rounded-2xl text-left dark:bg-gray-700">
                                                ...
                                                <th class="p-4 font-medium text-black dark:text-white">{{ __('Action') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                    ...
                                                    <td class="p-4">
                                                        <div class="flex items-center">
                                                            @if ($employer->user->image)
                                                                <img class="w-10 h-10 rounded-full" src="{{ asset($employer->user->image) }}" alt="image">
                                                            @else
                                                                <img class="w-10 h-10 rounded-full" src="https://img.freepik.com/premium-vector/company-icon-simple-element-illustration-company-concept-symbol-design-can-be-used-web-mobile_159242-7784.jpg" alt="image">
                                                            @endif
                                                            <div class="ml-3">
                                                                <div class="text-base font-semibold">{{ $employer->employer_name }}</div>
                                                                <div class="text-sm text-gray-500">{{ $employer->user->email }}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td class="p-4">
                                                        <span class="text-base font-semibold">Employee ( {{ $employer->employee->count() }} )</span>
                                                    </td>
                                                    <td class="p-4">
                                                        <a href="{{ $employer->website }}" target="_blank" class="hover:text-purple-500">
                                                            {{ $employer->website }}
                                                        </a>
                                                    </td>
                                                    <td class="p-4">
                                                        <div class="flex items-center">
                                                            <div class="h-2.5 w-2.5 rounded-full mr-2" style="background-color: {{ $employer->status === 1 ? 'green' : 'red' }};"></div>
                                                            <form id="statusForm{{ $employer->id }}" action="{{ route('employer.updateStatus', $employer->id) }}" method="post">
                                                                @csrf
                                                                <select name="status" 
                                                                    class="border-none bg-transparent focus:outline-none"
                                                                    onchange="document.getElementById('statusForm{{ $employer->id }}').submit()">
                                                                    <option value="1" {{ $employer->status === 1 ? 'selected' : '' }}>{{ __('Active') }}</option>
                                                                    <option value="0" {{ $employer->status === 0 ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                                                                </select>
                                                            </form>
                                                        </div>
                                                    </td>
