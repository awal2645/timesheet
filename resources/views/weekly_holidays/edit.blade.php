...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6 mt-6">
        <x-breadcrumb :items="[
            ...
            [
                'label' => __('Edit Weekly Holiday #' . $holiday->id),
                'icon' => true
            ]
        ]" />
    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-slate-800 shadow-2xl rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700">
            <!-- Form Header -->
            <div class="bg-gradient-to-r from-amber-50 to-orange-100 dark:from-slate-700 dark:to-slate-800 px-8 py-8 border-b border-gray-200 dark:border-gray-600">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                            <div class="w-8 h-8 bg-amber-100 dark:bg-amber-900/30 rounded-lg flex items-center justify-center mr-4">
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Modify the recurring weekly holiday configuration. Changes will apply to future occurrences.') }}
                        </p>
                    </div>
                    ...
                    <div class="hidden md:flex flex-col items-end space-y-2">
                        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
                            <span>{{ __('Holiday ID') }}</span>
                            <div class="bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-3 py-1 rounded-full font-semibold">
                                #{{ $holiday->id }}
                            </div>
                        </div>
                        <div class="text-xs text-gray-400 dark:text-gray-500">
                            {{ __('Created') }}: {{ $holiday->created_at ? $holiday->created_at->format('M d, Y') : __('N/A') }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Content -->
            <div class="p-8">
                <form action="{{ route('weekly_holidays.update', $holiday->id) }}" method="POST" class="space-y-8" id="editWeeklyHolidayForm">
            @csrf
            @method('PUT')
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-blue-700 dark:text-blue-400">
                                    @php
                                        ...
                                        $currentDays = array_map('ucfirst', $selectedDays);
                                    @endphp
                                    <p>{{ __('Currently set for') }}: <strong>{{ implode(', ', $currentDays) }}</strong></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-gray-200 dark:border-gray-600 pb-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
                                ...
                                {{ __('Update Weekly Recurring Days') }}
                            </h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('Modify which days of the week should be considered holidays on a recurring basis') }}</p>
                        </div>
                  ...
                  @php
                        $selectedDays = is_array($holiday->days_of_week) ? $holiday->days_of_week : json_decode($holiday->days_of_week);
                                    ...
                                    $selectedDaysLower = array_map('strtolower', $selectedDays);
                    @endphp
                <select id="days_of_week" name="days_of_week[]"
                                        class="select2 w-full px-4 py-4 border-2 border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all duration-200 hover:border-gray-400 dark:hover:border-gray-500 group-hover:shadow-md"
                                        multiple required>
                                    <option value="Monday" {{ in_array('monday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Monday') }}</option>
                                    <option value="Tuesday" {{ in_array('tuesday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Tuesday') }}</option>
                                    <option value="Wednesday" {{ in_array('wednesday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Wednesday') }}</option>
                                    <option value="Thursday" {{ in_array('thursday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Thursday') }}</option>
                                    <option value="Friday" {{ in_array('friday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Friday') }}</option>
                                    <option value="Saturday" {{ in_array('saturday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Saturday') }}</option>
                                    <option value="Sunday" {{ in_array('sunday', $selectedDaysLower) ? 'selected' : '' }}>{{ __('Sunday') }}</option>
                                </select>
                            </div>
                    ...
                    <!-- Change Tracking -->
                    <div id="changeTracker" class="hidden bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-6">
                        <div class="flex items-start">
                            <div class="flex-shrink-0">
                            ...
                            <div class="ml-4">
                                ...
                                <div id="changesList" class="text-sm text-amber-700 dark:text-amber-400">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Help Section -->
                    <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-6 mt-8">
                        <div class="flex items-start">
                            <div class="flex-shrink-0">
                                <svg class="w-6 h-6 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-amber-700 dark:text-amber-400 space-y-2">
                                    <p class="flex items-start">
                                        ...
                                        {{ __('Changes will apply to all future weekly occurrences') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ __('Past holiday instances remain unchanged') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ __('Employee schedules will be automatically updated') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between pt-8 border-t border-gray-200 dark:border-gray-600 space-y-4 sm:space-y-0">
                        ...
                        <button type="submit" id="submitBtn"
                            ...
                            <span class="submit-text">{{ __('Update Holiday') }}</span>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const form = document.getElementById('editWeeklyHolidayForm');
        ...
        const originalDays = @json(array_map('strtolower', is_array($holiday->days_of_week) ? $holiday->days_of_week : json_decode($holiday->days_of_week)));
        ...
        // Initialize Select2 with enhanced styling
            if (typeof jQuery !== 'undefined') {
                ...
                templateSelection: function(option) {
                    ...
                    return $(`
                        <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">
                            ${option.text}
                        </span>
                    `);
                }
                ...
                container.addClass('ring-2 ring-amber-500/20');
                setTimeout(() => {
                    container.removeClass('ring-2 ring-amber-500/20');
                }, 300);
        ...
        function trackChanges() {
            const currentDays = $('#days_of_week').val().map(day => day.toLowerCase());
            const added = currentDays.filter(day => !originalDays.includes(day));
            const removed = originalDays.filter(day => !currentDays.includes(day));
            
            if (added.length > 0 || removed.length > 0) {
                let changes = [];
                if (added.length > 0) {
                    changes.push(`✅ {{ __('Added') }}: ${added.map(day => day.charAt(0).toUpperCase() + day.slice(1)).join(', ')}`);
                }
                if (removed.length > 0) {
                    changes.push(`❌ {{ __('Removed') }}: ${removed.map(day => day.charAt(0).toUpperCase() + day.slice(1)).join(', ')}`);
                }
        ...
        window.selectWeekdays = function() {
            $('#days_of_week').val(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']).trigger('change');
            showNotification('{{ __('Weekdays selected') }}', 'success');
        };
