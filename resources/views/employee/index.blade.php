    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="mb-6 lg:mb-0">
                        ...
                        <p class="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
                            {{ __('Comprehensive dashboard to manage, track, and analyze all your employees and their activities') }}
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 text-center">
                            <div class="text-2xl font-bold text-red-600 dark:text-red-400 mb-1">
                                {{ $employees->where('status', 0)->count() }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                {{ __('Inactive') }}
                            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-8">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex items-center space-x-3">
                            @if(request()->hasAny(['search', 'status', 'employer_id']))
                ...
                <form action="{{ route('employee.index') }}" method="GET" class="p-6">
                    <!-- Main Search Row -->
                    ...
                    <div id="advancedFilters" class="hidden border-t border-gray-200 dark:border-gray-700 pt-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- Leave Range -->
                            <div>
                                <label for="min_leave" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('Min Leave Days') }}
                                </label>
                                <input type="number" 
                                       id="min_leave" 
                                       name="min_leave" 
                                       value="{{ request('min_leave') }}"
                                       placeholder="0"
                            ...
                            <div>
                                <label for="max_leave" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('Max Leave Days') }}
                                </label>
                                <input type="number" 
                                       id="max_leave" 
                                       name="max_leave" 
                                       value="{{ request('max_leave') }}"
                                       placeholder="30"
                                       class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" />
                            </div>
                            ...
                            <div>
                                ...
                                <select name="employer_id" id="employer_id" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="">{{ __('All Employers') }}</option>
                                    @foreach($employees->pluck('employer')->unique('id') as $employer)
                        ...
                        <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <div class="flex flex-wrap gap-2">
                                ...
                                <button type="button" class="quick-filter px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-filter="high-leave">
                                    {{ __('High Leave (20+ Days)') }}
                                </button>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 w-12 h-12 relative">
                                                ...
                                                @endif
                                                <div class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full {{ $employee->status === 1 ? 'bg-green-500' : 'bg-red-500' }} border-2 border-white dark:border-gray-800"></div>
                                            </div>
                                            <div class="ml-4">
                                        ...
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div class="text-center">
                                                <div class="text-lg font-bold text-green-600 dark:text-green-400">
                                                    {{ ($employee->total_leave ?? 0) - ($employee->approvedLeaveCount() ?? 0) }}
                                                </div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                                    {{ __('Remaining') }}
                                                </div>
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div class="relative">
                                                <input type="checkbox" 
                                                       ...
                                                       onchange="toggleStatus({{ $employee->id }}, this.checked)">
                                                <label for="status-toggle-{{ $employee->id }}" 
                                                       class="flex items-center cursor-pointer">
                                                    <div class="relative">
                                                        <!-- Toggle Background -->
                                                        <div class="toggle-bg w-12 h-6 rounded-full shadow-inner transition-colors duration-200 ease-in-out {{ $employee->status === 1 ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600' }}"></div>
                                                        <!-- Toggle Circle -->
                                                        <div class="toggle-dot absolute top-0.5 left-0.5 bg-white w-5 h-5 rounded-full shadow transition-transform duration-200 ease-in-out {{ $employee->status === 1 ? 'transform translate-x-6' : '' }}"></div>
                                                    </div>
                                                </label>
                                            </div>
                                            
                                            <!-- Status Badge -->
                                            <span class="status-badge inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $employee->status === 1 ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' }}">
                                                <div class="status-dot w-2 h-2 rounded-full mr-1.5 {{ $employee->status === 1 ? 'bg-green-500' : 'bg-red-500' }}"></div>
                                                <span class="status-text">
                                                    {{ $employee->status === 1 ? __('Active') : __('Inactive') }}
                                                </span>
                                            </span>
                                                                </div>
                                    ...
                                    <td colspan="5" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('No employees found') }}</h3>
                                            <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm">{{ __('You haven\'t added any employees yet. Get started by creating your first employee.') }}</p>
                                            <a href="{{ route('employee.create') }}"
                                               class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-200">
                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                                </svg>
                                                {{ __('Create First Employee') }}
                                            </a>
                                        </div>
                                                        </td>
                                                    </tr>
    ...
    <script>
        function showConfirmation(id) {
            Swal.fire({
                ...
                text: "{{ __('You want to delete this employee. This action cannot be undone!') }}",
                icon: 'warning',
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateActiveFilters() {
                ...
                const minLeave = document.getElementById('min_leave') ? document.getElementById('min_leave').value : '';
                const maxLeave = document.getElementById('max_leave') ? document.getElementById('max_leave').value : '';
                const employerId = document.getElementById('employer_id') ? document.getElementById('employer_id').value : '';
                ...
                if (minLeave) filters.push({ label: `Min Leave: ${minLeave}`, field: 'min_leave', value: minLeave });
                if (maxLeave) filters.push({ label: `Max Leave: ${maxLeave}`, field: 'max_leave', value: maxLeave });
                if (employerId) {
                    const employerSelect = document.getElementById('employer_id');
                    const employerName = employerSelect.options[employerSelect.selectedIndex].text;
                    filters.push({ label: `Employer: ${employerName}`, field: 'employer_id', value: employerId });
                }
        ...
        function toggleStatus(employeeId, isChecked) {
            console.log('toggleStatus called:', employeeId, isChecked);
            
            const statusValue = isChecked ? 1 : 0;
            const form = document.getElementById(`statusForm${employeeId}`);
            const checkbox = document.getElementById(`status-toggle-${employeeId}`);
