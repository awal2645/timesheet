<div
    ...
    <div class="px-5 py-5 pt-5">
        ...
        @if (auth('web')->user()->role != 'employer' && auth('web')->user()->role != 'employee' )
            <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">Employer(s)</h2>
            <div class="flex items-start">
                <div class="text-3xl font-bold text-slate-800 dark:text-slate-100 mr-2">{{ employerCount() }}</div>
            </div>
        @else
            <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">
                Report(s)</h2>
            <div class="flex items-start">
                <div class="text-3xl font-bold text-slate-800 dark:text-slate-100 mr-2">{{ reportCount() }}</div>
            </div>
        @endif
