    ...
    <div class="px-5 py-5 pt-5">
        ...
        @if (auth('web')->user()->role == 'employee')
            ...
            <div class="flex items-start">
                <div class="text-3xl font-bold text-slate-800 dark:text-slate-300 me-2">{{ reportCount('pending') }}
                </div>
            </div>
        @elseif (auth('web')->user()->role == 'client')
            ...
            <div class="flex items-start">
                <div class="text-3xl font-bold text-slate-800 dark:text-slate-300 me-2">{{ taskCount('pending') }}</div>
            </div>
        @else
            <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">{{ __('Employee(s)') }}</h2>
            <div class="flex items-start">
                <div class="text-3xl font-bold text-slate-800 dark:text-slate-300 me-2">{{ employeeCount() }}</div>
            </div>
        @endif
    </div>
</div>
