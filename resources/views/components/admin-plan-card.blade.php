...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 text-center flex flex-col justify-between h-full">
    ...
    @if ($plan->recommended)
        <div class="mt-2">
            <span class="bg-green-200 text-green-700 dark:bg-green-700 dark:text-green-200 p-3 rounded-full text-xs">{{ __('Recommended') }}</span>
        </div>
    @endif
    ...
    <ul class="my-4 space-y-2 text-center">
        <li class="flex items-center space-x-2">
            ...
            <span class="text-gray-800 dark:text-gray-100">{{ __('Employee Limit: :limit', ['limit' => $plan->employee_limit]) }}</span>
        </li>
        <li class="flex items-center space-x-2">
            <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m2 0a2 2 0 11-4 0 2 2 0 014 0z"></path>
            </svg>
            <span class="text-gray-800 dark:text-gray-100">{{ __('Client Limit: :limit', ['limit' => $plan->client_limit]) }}</span>
        </li>
        <li class="flex items-center space-x-2">
            <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m2 0a2 2 0 11-4 0 2 2 0 014 0z"></path>
            </svg>
            <span class="text-gray-800 dark:text-gray-100">{{ __('Project Limit: :limit', ['limit' => $plan->project_limit]) }}</span>
        </li>
    ...
    <div class="flex justify-between items-center">
        <a href="{{ route('plans.edit', $plan->id) }}" class="text-white bg-teal-500 hover:bg-teal-600 rounded-lg px-4 py-2">
        ...
        <a href="javascript:void(0);" onclick="showConfirmation({{ $plan->id }})" class="text-white bg-red-500 hover:bg-red-600 rounded-lg px-4 py-2">
