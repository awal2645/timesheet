        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10"> <!-- Use grid layout for forms -->
            ...
            <form action="{{ route('payment.update') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="flex items-center justify-between">
                    <h3
                        class="text-xl font-bold mb-4 flex items-center justify-between text-text-light dark:text-text-dark">
                        <!-- Title and Icon -->
                        <div class="flex items-center">
                            <i class="fab fa-paypal me-2" style="color: var(--primary-50);"></i>
                            {{ __('Paypal Settings') }}
                        </div>
                    ...
                    <div class="mt-2 flex justify-end">
                        <a href="https://developer.paypal.com/developer/accounts/" target="_bulk"
                            class="text-primary-50 dark:text-primary-50 hover:underline"
                ...
                <button type="submit"
                    class="text-text-light dark:text-text-dark bg-primary-50 hover:bg-primary-300 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm w-full sm:w-auto px-6 py-3 tewhitext-center dark:bg-primary-500 dark:hover:bg-primary-700 dark:focus:ring-primary-800">{{ __('Submit') }}</button>
            ...
            <form action="{{ route('payment.update') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="flex items-center justify-between">
                    <h3 class="text-xl font-bold mb-4 flex items-center text-text-light dark:text-text-dark">
                        <i class="fab fa-stripe me-2 text-primary-50" style="color: var(--primary-50);"></i>
                        {{ __('Stripe Settings') }}
                    </h3>
                    <a href="https://docs.stripe.com/keys" target="_bulk"
                        class="text-primary-50 dark:text-primary-50 text-sm"
                        style="text-decoration: underline; color: var(--primary-50);  ">({{ __('Get Help') }})</a>
                </div>
                ...
                <button type="submit"
                    class="text-text-light dark:text-text-dark bg-primary-50 hover:bg-primary-300 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm w-full sm:w-auto px-6 py-3 text-center dark:bg-primary-500 dark:hover:bg-primary-700 dark:focus:ring-primary-800">{{ __('Submit') }}</button>
                <!-- Consistent button style -->
            </form>
        </div>
    </div>
