...
<x-app-layout>
    ...
    <div class="m-6 card">
        ...
        <form action="{{ route('notices.update', $notice->id) }}" method="POST">
            ...
            <div class="form-field">
                <select multiple id="role" name="role[]" class="select2">
                    ...
                    @endphp
                    <option class="dark:bg-slate-800 text-text-light dark:text-text-dark" value="{{ $role->id }}" {{ in_array($role->id, $noticeRoles) ? 'selected' : '' }}>
                        {{ ucfirst($role->name) }}
                    </option>
                    @endforeach
                </select>
