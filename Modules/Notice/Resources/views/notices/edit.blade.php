...
<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-slate-800 shadow-2xl rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Update notice information. Changes will be visible to all targeted user roles immediately.') }}
                        </p>
                    </div>
                    ...
                    <div class="hidden md:flex flex-col items-end space-y-2">
                        ...
                        <div class="text-xs text-gray-400 dark:text-gray-500">
                            {{ __('Created') }}: {{ $notice->created_at ? $notice->created_at->format('M d, Y') : __('N/A') }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Content -->
            <div class="p-8">
                <form action="{{ route('notices.update', $notice->id) }}" method="POST" class="space-y-8" id="editNoticeForm">
            @csrf
            @method('PUT')
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                <h3 class="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">
                                    {{ __('Current Notice Details') }}
                                </h3>
                                <div class="text-sm text-blue-700 dark:text-blue-400">
                                    <p><strong>{{ __('Title') }}:</strong> {{ $notice->title }}</p>
                                    <p><strong>{{ __('Content') }}:</strong> {{ Str::limit($notice->content, 100) }}</p>
                                    <p><strong>{{ __('Target Roles') }}:</strong> 
                                        @php
                                            $noticeRoles = explode(',', $notice->role);
                                            $roleNames = $roles->whereIn('id', $noticeRoles)->pluck('name')->map(function($name) {
                                                return ucfirst($name);
                                            })->join(', ');
                                        @endphp
                                        {{ $roleNames ?: __('No roles selected') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="space-y-6">
                        <div class="border-b border-gray-200 dark:border-gray-600 pb-4">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('Update the notice details and target audience') }}</p>
                        </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    <textarea name="content" id="content" rows="6" required
                                              ...
                                              placeholder="Write your notice content here. Be clear and provide all necessary details...">{{ old('content') ?? $notice->content }}</textarea>
                                </div>
                                @error('content')
                                <div class="flex items-center mt-2 p-3 text-red-700 bg-red-50 dark:bg-red-900/20 dark:text-red-400 rounded-lg border border-red-200 dark:border-red-800">
                                    <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @php
                        $noticeRoles = explode(',', $notice->role);
                                            $isSelected = in_array($role->id, $noticeRoles) || in_array($role->id, old('role', []));
                    @endphp
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-orange-700 dark:text-orange-400 space-y-2">
                                    <p class="flex items-start">
                                        ...
                                        {{ __('Changes will be immediately visible to all targeted user roles') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ __('Consider the urgency and importance when updating content') }}
                                    </p>
                                    <p class="flex items-start">
                                        <svg class="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                        </svg>
                                        {{ __('Changing target roles will affect who can see this notice') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between pt-8 border-t border-gray-200 dark:border-gray-600 space-y-4 sm:space-y-0">
                        ...
                        <button type="submit" id="submitBtn"
                            ...
                            <span class="submit-text">{{ __('Update Notice') }}</span>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const originalRoles = [{{ implode(',', explode(',', $notice->role)) }}].map(String);
        ...
        function trackChanges() {
            ...
            const currentRoles = $('#role').val() || [];
            
            let changes = [];
            
            if (currentTitle !== originalTitle) {
                changes.push(`📝 {{ __('Title changed') }}: "${originalTitle}" → "${currentTitle}"`);
            }
            
            if (currentContent !== originalContent) {
                const oldContentPreview = originalContent.length > 50 ? originalContent.substring(0, 50) + '...' : originalContent;
                const newContentPreview = currentContent.length > 50 ? currentContent.substring(0, 50) + '...' : currentContent;
                changes.push(`📄 {{ __('Content changed') }}: "${oldContentPreview}" → "${newContentPreview}"`);
            }
            
            // Check role changes
            const rolesAdded = currentRoles.filter(role => !originalRoles.includes(role));
            const rolesRemoved = originalRoles.filter(role => !currentRoles.includes(role));
            
            if (rolesAdded.length > 0 || rolesRemoved.length > 0) {
                let roleChangeText = '👥 {{ __('Target roles changed') }}:';
                if (rolesAdded.length > 0) {
                    const addedNames = rolesAdded.map(roleId => {
                        const option = $(`#role option[value="${roleId}"]`);
                        return option.text();
                    });
                    roleChangeText += ` +${addedNames.join(', ')}`;
                }
                if (rolesRemoved.length > 0) {
                    const removedNames = rolesRemoved.map(roleId => {
                        const option = $(`#role option[value="${roleId}"]`);
                        return option.text();
                    });
                    roleChangeText += ` -${removedNames.join(', ')}`;
                }
                changes.push(roleChangeText);
            }
