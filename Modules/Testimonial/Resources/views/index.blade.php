    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="mb-6 lg:mb-0">
                        ...
                        <p class="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
                            {{ __('Manage customer testimonials and reviews to showcase your business success and build trust with potential clients') }}
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 text-center">
                            <div class="text-2xl font-bold text-orange-600 dark:text-orange-400 mb-1">
                                {{ $testimonials->unique('company')->count() }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                {{ __('Companies') }}
                            </div>
            ...
            {{ __('Add New Testimonial') }}
                ...
                <form action="{{ route('testimonial.index') }}" method="GET" class="p-6">
                    <!-- Main Search Row -->
                        ...
                        <div class="lg:col-span-8">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" 
                                       ...
                                       placeholder="{{ __('Search by name, company, or designation...') }}" />
                            </div>
                        </div>
                ...
                {{ __('Search') }}
                    ...
                    <div id="advancedFilters" class="hidden border-t border-gray-200 dark:border-gray-700 pt-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- Rating Filter -->
                            <div>
                                <label for="rating_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('Filter by Rating') }}
                                </label>
                                <select name="rating_filter" id="rating_filter" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    <option value="">{{ __('All Ratings') }}</option>
                                    <option value="5" {{ request('rating_filter') == '5' ? 'selected' : '' }}>{{ __('5 Stars') }}</option>
                                    <option value="4" {{ request('rating_filter') == '4' ? 'selected' : '' }}>{{ __('4 Stars') }}</option>
                                    <option value="3" {{ request('rating_filter') == '3' ? 'selected' : '' }}>{{ __('3 Stars') }}</option>
                                    <option value="2" {{ request('rating_filter') == '2' ? 'selected' : '' }}>{{ __('2 Stars') }}</option>
                                    <option value="1" {{ request('rating_filter') == '1' ? 'selected' : '' }}>{{ __('1 Star') }}</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="company_filter" id="company_filter" class="block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                    ...
                                    @foreach($testimonials->unique('company')->pluck('company')->filter() as $company)
                        ...
                        <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <div class="flex flex-wrap gap-2">
                                ...
                                <button type="button" class="quick-filter px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-filter="five-star">
                                    {{ __('5-Star Reviews') }}
                                </button>
                                ...
                                <button type="button" class="quick-filter px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-filter="high-rating">
                                    {{ __('High Ratings (4+)') }}
            </button>
                            </div>
                        </div>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <div class="flex items-start">
                                            <div class="flex-shrink-0 w-12 h-12">
                                                @if($testimonial->image)
                                                    <img src="{{ asset($testimonial->image) }}" alt="{{ $testimonial->name }}" class="w-12 h-12 rounded-full object-cover border-2 border-gray-200 dark:border-gray-600">
                                                @else
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-sm font-medium text-gray-900 dark:text-white">
                                                    {{ $testimonial->designation ?? __('N/A') }}
                                                </span>
                                            </div>
                                            <div class="flex items-center">
                                                <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {{ $testimonial->rating }}/5
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ __('Added:') }} {{ $testimonial->created_at ? $testimonial->created_at->format('M d, Y') : __('Unknown') }}
                                            </div>
                                        </div>
                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('No testimonials found') }}</h3>
                                            <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm">{{ __('No customer testimonials have been added yet. Start building trust by collecting your first testimonial.') }}</p>
                                            <a href="{{ route('testimonial.create') }}"
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-700 dark:text-gray-300">
                                {{ __('Showing') }} 
                                <span class="font-medium">{{ $testimonials->firstItem() }}</span>
                                {{ __('to') }}
                                <span class="font-medium">{{ $testimonials->lastItem() }}</span>
                                {{ __('of') }}
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateActiveFilters() {
                ...
                if (ratingFilter) {
                    filters.push({ label: `Rating: ${ratingFilter} Star${ratingFilter > 1 ? 's' : ''}`, field: 'rating_filter', value: ratingFilter });
                }
                if (companyFilter) {
